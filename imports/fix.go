...
// bubbleSortLocalPackages will bubble to the top of the list all packages
// which are local to the package package in pkgDir
func bubbleSortLocalPackages(pkgDir string, pkgs []*pkg) {

	// find the package path
	importPath := pkgDirToPkgImportPath(pkgDir)
	if importPath == "" {
		return
	}
	var top int
	for i := 0; i < len(pkgs); i++ {
		if strings.HasPrefix(pkgs[i].importPathShort, importPath) {
			for j := i; j > top; j-- {
...
func findCanidates(pkgDir string) []*pkg {
...
func walkCandidates(candidates []*pkg, pkgDir, pkgName string, symbols map[string]bool) (importPath string, rename, ok bool) {
	...
	// Sort by bubbling up packages which are local to the package
	// that contains filename.  It is assumed that a local package
	// is much more likely to be correct rather then the package
	// with the shortest name.  The shortest name sort is retained
	// as the second sort order.
	bubbleSortLocalPackages(pkgDir, candidates)
